 ...
 * @param templateName - Name of the approved template (e.g., "otp_verification")
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorCode === 190 || errorMessage?.includes("Malformed access token") || errorMessage?.includes("Invalid OAuth access token")) {
        ...
        errorMessage = "Template not found or not approved. Please check the template name in Meta Business Manager."
 ...
 * Note: This only works within the 24-hour window. Use sendTemplateMessage for OTP and other authentication messages.
 */
