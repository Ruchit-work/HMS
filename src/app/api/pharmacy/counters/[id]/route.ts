...
export async function PATCH(
  ...
  const init = initFirebaseAdmin('pharmacy/counters/[id]/PATCH')
  if (!init.ok) {
  ...
  return NextResponse.json({ success: true, counter: { id, ...updated } })
}
...
export async function DELETE(
  ...
  const init = initFirebaseAdmin('pharmacy/counters/[id]/DELETE')
  if (!init.ok) {
    return NextResponse.json({ success: false, error: init.error || 'Server not configured' }, { status: 500 })
  }
