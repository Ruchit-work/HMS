...
import { getHospitalCollectionPath, getUserActiveHospitalId } from "@/shared/utils/firebase/serverHospitalQueries"
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!catalogMode && queryText.length < 2) return Response.json({ medicines: [] })
    ...
    return Response.json(
      ...
      {
        headers: {
          // Catalog mode is loaded once by the consultation UI and searched locally.
          // Private caching avoids repeated Firestore reads when the form remounts.
          "Cache-Control": catalogMode ? "private, max-age=60" : "private, no-store",
        },
      }
    )
  } catch (error: any) {
