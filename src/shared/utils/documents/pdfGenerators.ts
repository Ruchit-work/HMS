...
function createPrescriptionDocument(appointment: Appointment, options: PrescriptionRenderOptions = {}) {
  ...
  if (structuredPrescription && structuredPrescription.medicines.length > 0) {
    ...
    // Consultation completed without prescribing medicines
    medicines.push({
      desc: "No new medications prescribed."
    })
  }
...
function buildPrescriptionHTML(appointment: Appointment): string {
  ...
  const fallbackRow = `
    <tr>
      <td class="num">1</td>
      <td>${escapeHtmlText(sanitizeForPdf(appointment.medicine || "No new medications prescribed."))}</td>
      <td>—</td>
      <td>${appointment.medicine ? "As advised" : "—"}</td>
      <td>${appointment.medicine ? "As advised" : "—"}</td>
    </tr>
  `
