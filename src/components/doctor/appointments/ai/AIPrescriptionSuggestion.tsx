...
export default function AIPrescriptionSuggestion({
  ...
  if (isLoading) {
    return (
      <div className="mb-2 bg-slate-50 border border-slate-200 rounded px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-slate-700">Generating suggestions...</span>
        </div>
      </div>
    )
  }
  ...
  if (!isVisible || !aiPrescriptionText) {
    if (isVisible) {
      return (
        <div className="mb-2 bg-slate-50 border border-slate-200 rounded px-3 py-2">
          <p className="text-xs text-slate-600">Prescription suggestions are being prepared. Please wait.</p>
        </div>
      )
    }
    return null
  ...
  return (
    <div className="mb-2 space-y-2">
      ...
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <h4 className="text-xs font-semibold text-slate-700 uppercase tracking-wide">Suggested medicines</h4>
        <div className="flex items-center gap-1.5">
          {visibleMedicines.length > 0 && (
            <>
              <button
                ...
                className="px-2 py-1 text-[11px] font-semibold text-slate-700 bg-white border border-slate-300 rounded hover:bg-slate-50 transition-colors"
              >
                Add all
              </button>
              <button
          ...
          <button
            ...
            disabled={isLoading}
            className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-semibold text-blue-700 bg-blue-50 border border-blue-200 rounded hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {visibleMedicines.length > 0 ? (
                ...
                {isPendingRemove ? (
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      <button
                        ...
                        className="px-2 py-1 text-xs font-medium text-white bg-red-600 border border-red-700 rounded hover:bg-red-700"
                      >
                        Remove suggestion
                      </button>
                    </div>
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex-1 min-w-0 flex flex-wrap items-baseline gap-x-3 gap-y-0.5 text-xs">
                      ...
                      {med.dosage && <span className="text-slate-600">{med.dosage}</span>}
                      {med.frequency && <span className="text-slate-600">{med.frequency}</span>}
                      {med.duration && <span className="text-slate-600">{med.duration}</span>}
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      {!isAlreadyAdded ? (
                        <button
                          ...
                          className="px-2 py-1 text-[11px] font-semibold text-white bg-blue-600 border border-blue-700 rounded hover:bg-blue-700 transition-colors"
                        >
                          Add
                        </button>
                      ) : (
                        <span className="px-2 py-1 text-[11px] font-medium text-slate-600 bg-slate-100 border border-slate-200 rounded">
                          Added
                        </span>
                      )}
                      <button
        ...
        <div className="bg-white border border-slate-200 rounded px-3 py-2">
          <p className="text-xs text-slate-600">No suggestions available or all have been added.</p>
        </div>
      )}
    </div>
  )
