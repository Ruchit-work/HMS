...
import { getDayName, generateTimeSlots, isSlotInPast, DEFAULT_VISITING_HOURS } from "@/shared/utils/timeSlots"
      ...
      try {
        const result = await computeAvailableSlots({
          ...
          doctor: {
            ...
            blockedDates: doctorProfile?.blockedDates,
          } as any,
        })
        setAvailableSlots(result.available)
      } catch (error) {
        console.error("[DoctorBookAppointmentPage] Failed to compute available slots:", error)
        setAvailableSlots([])
      }
    }
    compute()
