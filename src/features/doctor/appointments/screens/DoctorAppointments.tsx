...
import { fetchMedicineSuggestions, MedicineSuggestion, recordMedicineSuggestions } from "@/shared/utils/medicineSuggestions"
...
import { useDoctorAppointments } from "@/shared/hooks/useDoctorAppointments"
import { useDoctorBranches } from "@/shared/hooks/useDoctorBranches"
...
import { useAppointmentFilters } from "@/shared/hooks/useAppointmentFilters"
import {
  ClinicalPageFrame,
  ConsultationLayout,
  ReportViewer,
...
function DoctorAppointmentsContent() {
  ...
  const runCompletionFlow = async (
    ...
    // Ignore rows without a medicine name (medicine is optional).
    const validMedicines = (formData.medicines || []).filter((med) => med.name && med.name.trim())
    const medicineText = formatMedicinesAsText(validMedicines, submissionNotes)
      ...
      try {
        ...
        if (token && appointmentSnapshot) {
          const res = await fetch("/api/doctor/create-appointment", {
            ...
            body: JSON.stringify({
              appointmentData: {
                ...
                medicalHistory: "",
                status: "confirmed",
                // Fee is resolved server-side from the doctor's consultationFee.
                // Rechecks are always created as unpaid with no payment method —
                // front desk records the method when the money is actually collected.
                paymentType: "full",
          ...
          if (!res.ok) {
            ...
            console.error("[recheckup] auto-book failed:", errBody?.error || res.status)
            setNotification({
              type: "error",
              message:
                "Consultation completed, but the re-checkup could not be booked automatically. Please book it manually.",
            })
          }
        }
      } catch (err) {
        console.error("[recheckup] auto-book error:", err)
        setNotification({
          type: "error",
          message:
            "Consultation completed, but the re-checkup could not be booked automatically. Please book it manually.",
  ...
  const handleCompleteAppointment = async (
    ...
    // Medicine is optional — only clinical documentation is required.
  ...
  const confirmAdmitPatient = async () => {
    ...
    try {
      // Medicine is optional — the consultation just needs to be finalized
      // (with clinical documentation) before admitting.
      const alreadyCompleted =
        appointment?.status === "completed" ||
        Boolean(appointment?.medicine && appointment.medicine.trim()) ||
        Boolean(appointment?.doctorNotes && String(appointment.doctorNotes).trim())

      const formData = completionData[appointmentId]

      if (!alreadyCompleted) {
        if (!formData || !hasClinicalDocumentation(formData)) {
          setNotification({
            type: "error",
            message: "Please enter clinical notes or diagnosis before admitting the patient.",
          })
          setAdmitting((prev) => ({ ...prev, [appointmentId]: false }))
          closeAdmitDialog()
          return
        ...
        try {
          ...
          setNotification({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Failed to save consultation before admitting. Please try again.",
          })
          setAdmitting((prev) => ({ ...prev, [appointmentId]: false }))
          closeAdmitDialog()
          return
                        ...
                        <CompletionForm
                          ...
                          actionBar={
                            <ConsultationActionBar
                              ...
                              canComplete={hasClinicalDocumentation(
                                completionData[selectedAppointment.id] || {
                                  medicines: [],
                                  notes: "",
                                  recheckupRequired: false,
