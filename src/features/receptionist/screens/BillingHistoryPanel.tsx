...
export default function BillingHistoryPanel({
  ...
  const statusFilteredRecords = useMemo(() => {
    ...
    return billingRecords.filter((record) => {
      if (billingStatusFilter === "pending") {
        // Cancelled/refunded invoices are closed, not collectable dues.
  ...
  const handleOpenBillingPayment = useCallback(
    (record: BillingRecord) => {
      // Only open collection for records that actually have money owed.
      if (record.status === "paid" || record.status === "void" || record.status === "cancelled") return
      if (!billingSettings.billingOptions.allowManualPaymentEntry) {
        notify({
          type: "error",
          message: "Manual payment entry is disabled for this hospital.",
        })
        return
      }
      setSelectedBillingRecord(record)
      const preferred =
        record.paymentMethod && frontDeskPaymentMethods.includes(record.paymentMethod as BillingPaymentMethod)
          ? (record.paymentMethod as BillingPaymentMethod)
          : frontDeskPaymentMethods[0]
      setBillingPaymentMethod(preferred)
      setBillingPaymentData(emptyPaymentData)
      setBillingPaymentModalOpen(true)
    },
    [billingSettings.billingOptions.allowManualPaymentEntry, frontDeskPaymentMethods, notify]
  )
        ...
        {billingLoading && billingRecords.length === 0 ? (
          ...
          <div>
            {paginatedBillingRecords.map((record) => {
              ...
              const statusLabel = isPaid ? "Paid" : isVoid ? "Voided" : isCancelled ? "Cancelled" : "Pending"

              const accentColor = isPaid ? "bg-emerald-500" : isVoid || isCancelled ? "bg-red-400" : "bg-amber-400"
              const amountColor = isPaid ? "text-emerald-700" : isVoid || isCancelled ? "text-slate-500" : "text-amber-700"
              const amountBg = isPaid ? "bg-emerald-50 border-emerald-100" : isVoid || isCancelled ? "bg-slate-50 border-slate-200" : "bg-amber-50 border-amber-100"
                    ...
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      ...
                      <div className="flex-1 min-w-0 space-y-3">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          {refundsEnabled && Number(record.refundAmount || 0) > 0 && (
          ...
          <div className="flex w-full max-w-lg max-h-[min(92vh,720px)] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-4 sm:px-6 sm:py-5 space-y-4">
              ...
              <PaymentMethodSection
                ...
                methods={frontDeskPaymentMethods}
              />
            </div>
