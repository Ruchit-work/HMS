...
import { filterPatientsByBranch } from '@/shared/utils/branch/branchFilters'
...
export default function PatientManagement({
    ...
    legacyPatientsRef.current = legacyPatients
    ...
    const setupPatientsListener = useCallback(() => {
        ...
        try{
            ...
            const unsubscribe = onSnapshot(q, async (snapshot) => {
                ...
                const legacy = legacyPatientsRef.current
                if (legacy.length > 0) {
    ...
    // When legacy list updates, re-merge into current hospital patients without resubscribing.
    useEffect(() => {
        if (!activeHospitalId || legacyPatients.length === 0) return
        setPatients((prev) => {
    ...
    }, [legacyPatients, activeHospitalId])
