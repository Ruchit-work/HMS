...
export default function FinancialAnalytics() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        let records: UnifiedBillingRecord[] = []
        if (billingRes.ok) {
  ...
  const computeFinancialAnalytics = useCallback(() => {
    ...
    try {
      ...
      let appointments: Appointment[] = rawAppointmentsRef.current as unknown as Appointment[]
      ...
      let filteredBillingRecords = records
      let filteredAppointments = appointments
      
      if (timeRange === '7days') {
        filteredBillingRecords = records.filter(r => new Date(r.generatedAt) >= sevenDaysAgo)
        filteredAppointments = appointments.filter(a => new Date(a.createdAt) >= sevenDaysAgo)
      } else if (timeRange === '30days') {
        filteredBillingRecords = records.filter(r => new Date(r.generatedAt) >= thirtyDaysAgo)
        filteredAppointments = appointments.filter(a => new Date(a.createdAt) >= thirtyDaysAgo)
      } else if (timeRange === '3months') {
        filteredBillingRecords = records.filter(r => new Date(r.generatedAt) >= threeMonthsAgo)
        filteredAppointments = appointments.filter(a => new Date(a.createdAt) >= threeMonthsAgo)
      } else if (timeRange === '6months') {
        filteredBillingRecords = records.filter(r => new Date(r.generatedAt) >= sixMonthsAgo)
        filteredAppointments = appointments.filter(a => new Date(a.createdAt) >= sixMonthsAgo)
      } else if (timeRange === '1year') {
        filteredBillingRecords = records.filter(r => new Date(r.generatedAt) >= oneYearAgo)
      ...
      // Outstanding: include unpaid/partial even when paymentAmount is still 0
      // (e.g. unpaid recheckups / WhatsApp bookings that only set remainingAmount
      // or totalConsultationFee). Skip appointments already represented in billing records
      // so pending amounts are not double-counted.
      const appointmentIdsInBilling = new Set(
      ...
      filteredAppointments.forEach(apt => {
        if (appointmentIdsInBilling.has(apt.id)) return

        // Cancelled appointments are closed — nothing is collectable on them.
        ...
        if (paymentStatus === "refunded" || paymentStatus === "paid") return
        if (paymentStatus !== "pending" && paymentStatus !== "unpaid") return
        // Evidence of a completed collection wins over a stale unpaid flag.
        if (apt.paidAt && String(apt.paidAt).trim() !== "" && Number(apt.remainingAmount || 0) <= 0) return

        const owed =
          Number(apt.remainingAmount || 0) ||
          Number(apt.paymentAmount || 0) ||
          Number(apt.totalConsultationFee || 0) ||
          Number(apt.consultationFee || 0)
      ...
      // Revenue by doctor — billing records are the primary source (already include
      // paid appointments). Only add raw appointments that are not already
      // represented, so the same payment is never counted twice.
      ...
      const appointmentIdsWithBillingForRevenue = new Set(
        filteredBillingRecords
          .map((r) => r.appointmentId)
          .filter((id): id is string => Boolean(id))
      )

      // From billing records (paid only — refunded maps to cancelled upstream)
      filteredBillingRecords.forEach(record => {
      ...
      // From appointments not already covered by a billing record
      filteredAppointments.forEach(apt => {
  ...
  }, [activeHospitalId, billingRecords, selectedBranchId, timeRange])
