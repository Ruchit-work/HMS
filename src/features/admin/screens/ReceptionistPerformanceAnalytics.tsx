  ...
  const fetchReceptionistAnalytics = async () => {
    ...
    try {
      ...
      const receptionistStats: ReceptionistAnalytics[] = receptionistsList.map(rec => {
        ...
        // Revenue from receptionist-booked appointments that were actually collected.
        // Never count unpaid or refunded amounts.
        const receptionistAppointments = appointmentsByReceptionists
        const revenue = receptionistAppointments
          .filter((apt) => {
            const status = String(apt.paymentStatus || "").toLowerCase()
            if (status === "refunded") return false
            return (
              status === "paid" ||
              (Boolean(apt.paidAt) && String(apt.paidAt).trim() !== "")
            )
          })
          .reduce((sum, apt) => sum + (apt.paymentAmount || 0), 0) / Math.max(receptionistsList.length, 1)
